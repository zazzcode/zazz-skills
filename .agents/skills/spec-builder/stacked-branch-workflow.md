...
- **One stack inside that worktree** = multiple review branches for the same
  deliverable or tightly related deliverable group.
...
- **Stacked SPEC**: `<container-root>/<slug>-stacked-SPEC.md` (container root,
  shared across stacked branches).
- **Lane worktree dir**: `mw-<slug>-lane`.
- **Stacked branch names**: typically `mw-<slug>-svc-1`, `mw-<slug>-svc-2`.
- **Run log**: `<lane-worktree>/docs/implementation/<slug>-RUN-LOG.md`.

No PLAN files are created. Branch-specific execution phases, ACs, halt conditions, and
implementation prompts live in the stacked SPEC and run log.

## Required SPEC contents beyond the standard quality bar

The `stacked-SPEC-TEMPLATE.md` enforces these. Fill the template; don't reinvent the
section list.
...
1. Confirm `<slug>` (kebab-case; used in lane worktree, branch names, SPEC, and run log).
2. Confirm that stacked review is actually needed. If one PR can review the milestone,
   ...
   branches instead.
3. Resolve the SPEC path at the container root.
...
6. Read a project-local prior stacked SPEC if the Owner points to one for calibration.
...
8. Read `docs/standards/index.yaml` from the active lane worktree; load only relevant
   standards.
9. Copy `stacked-SPEC-TEMPLATE.md` to the container-root path and fill placeholders
...
## gh-stack rules to preserve in SPECs

When the stacked SPEC includes commands or implementation prompts, carry these
...
State this explicitly in the SPEC:

`mw-<slug>-svc-2` rebases upstack from `mw-<slug>-svc-1` until the svc-1 PR lands on
`dev` through human review. After the svc-1 PR lands, svc-2 rebases on `origin/dev`.
Never squash-rebase svc-2 onto `dev` directly while stacked; that absorbs svc-1's
commits into svc-2's PR and breaks the no-drift acceptance criterion.
